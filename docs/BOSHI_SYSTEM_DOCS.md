...
| 快速摘要模型 | deepseek-r1:1.5b | — | 结构化 JSON 输出 |
...
| 模型 | 用途 | 特性 | token 预算 |
|------|------|------|-----------|
| `deepseek-r1:1.5b` | 新闻摘要与分类 | CoT 在 `<think>` 标签内，`content` 字段包含实际输出，响应快（约 3s）| 700 |
...
使用 `deepseek-r1:1.5b`。Prompt 包含一个 few-shot 示例，要求模型返回固定结构的 JSON：
...
| GET | `/api/analysis/ollama/health` | Ollama 健康检查（确认 `deepseek-r1:1.5b` 可用）|
...
| `deepseek-r1:1.5b` 模型 | 1.1 GB | 新闻摘要翻译 |
| Python | 3.11+ | 后端运行时 |
| Node.js | 18+ | 前端构建工具 |
